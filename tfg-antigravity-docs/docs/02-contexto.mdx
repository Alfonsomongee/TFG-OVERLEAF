...
La ecuación de swing gobierna el comportamiento de la frecuencia ante perturbaciones de potencia activa ([*ver Simulador de Ecuación de Oscilación*](./galeria-graficas.mdx#swing)). Sin embargo, el 28-A certificó que el sistema tenía $H = 2,3$ s — por encima del umbral ENTSO-E — y aun así colapsó. El mecanismo relevante fue de naturaleza diferente: la pérdida de fortaleza de red medida por el SCR, que determinó la dinámica de tensión en el plano Q-V. Esto establece la jerarquía analítica del incidente: primero estabilidad de tensión, luego inercia.
