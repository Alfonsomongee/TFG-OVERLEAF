...
function getPowerPalette(isDark) {
  return {
    ...
    april28: isDark ? '#D98798' : '#A13D36',
    april29: isDark ? '#7DCDE3' : '#1F6F78',
...
export default function PotenciaChart() {
  ...
  const palette = getPowerPalette(isDark);
  ...
  return (
    ...
    >
      ...
      >
        {isEs ? 'Comparativa de Potencia por Tecnología (28 vs 29 Abril)' : 'Power Comparison by Technology (Apr 28 vs 29)'}
      </h3>
      <ResponsiveContainer width="100%" height="90%">
        <BarChart data={data} layout="vertical" margin={{ top: 20, right: 36, left: 8, bottom: 28 }}>
          ...
          <XAxis type="number" stroke={palette.axisLine} tick={{ fill: palette.axis, fontSize: 12, fontFamily: 'Space Mono' }} unit=" MW" />
          <YAxis
            type="category"
            dataKey="category"
          ...
          />
          <Bar dataKey="28 Abril" fill={palette.april28} radius={[0, 4, 4, 0]} barSize={12} />
          <Bar dataKey="29 Abril" fill={palette.april29} radius={[0, 4, 4, 0]} barSize={12} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
