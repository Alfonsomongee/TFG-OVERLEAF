...
with tab_forense:
    st.markdown("### 🔬 Análisis Forense — Comparativa Temporal")

    df_28a = cargar_datos_28a()
    snap   = st.session_state.ultimo_snapshot

    # ── Gráfica 28-A con fases anotadas ──
    if not df_28a.empty:
        st.markdown("#### 📈 Demanda del 28-A con fases del incidente")

        fig_28 = go.Figure()

        # Zona de colapso sombreada
        fig_28.add_vrect(
            x0="2025-04-28 12:33", x1="2025-04-28 13:10",
            fillcolor=COLOR_ZONA, layer="below", line_width=0,
            annotation_text="COLAPSO", annotation_position="top left",
            annotation_font=dict(color="#e74c3c", size=11),
        )
        # Zona Black Start
        fig_28.add_vrect(
            x0="2025-04-28 13:10", x1="2025-04-28 14:30",
            fillcolor="rgba(243,156,18,0.08)", layer="below", line_width=0,
            annotation_text="BLACK START", annotation_position="top left",
            annotation_font=dict(color="#f39c12", size=11),
        )

        # Curva demanda 28-A
        fig_28.add_trace(go.Scatter(
            x=df_28a["hora_dt"], y=df_28a["demand"],
            name="Demanda 28-A",
            line=dict(color=COLOR_28A, width=2.5),
            fill="tozeroy",
            fillcolor="rgba(231,76,60,0.07)",
            hovertemplate="%{x|%H:%M} → %{y:,.0f} MW<extra>28-A</extra>",
        ))

        # Línea de demanda actual (si hay datos)
        if snap.get("demanda"):
            fig_28.add_hline(
                y=snap["demanda"],
                line=dict(color=COLOR_HOY, dash="dash", width=1.5),
                annotation_text=f"Ahora: {snap['demanda']:,.0f} MW",
                annotation_font=dict(color=COLOR_HOY),
            )

        # Marcadores de eventos
        eventos = [
            ("2025-04-28 12:33", 25184, "⚡ Colapso\n25.184 MW"),
            ("2025-04-28 08:00", 28500, "Máx. matinal"),
        ]
        for ts_e, val_e, label_e in eventos:
            try:
                ts_dt = pd.to_datetime(ts_e)
                if ts_dt >= df_28a["hora_dt"].min() and ts_dt <= df_28a["hora_dt"].max():
                    fig_28.add_annotation(
                        x=ts_dt, y=val_e, text=label_e,
        ...
        fig_28.update_layout(
            ...
            plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#e8eaf0"),
            xaxis=dict(title="Hora", gridcolor="rgba(255,255,255,0.07)"),
            yaxis=dict(title="Demanda (MW)", gridcolor="rgba(255,255,255,0.07)"),
            legend=dict(bgcolor="rgba(0,0,0,0)"),
            margin=dict(l=0, r=0, t=20, b=0),
        )
        st.plotly_chart(fig_28, use_container_width=True)

    # ── Trayectoria Inercia vs RoCoF ──
    st.markdown("#### 🔵 Trayectoria Inercia vs RoCoF Simulado")

    col_slider, col_info = st.columns([2, 1])
    with col_slider:
        delta_p_slider = st.slider(
            "Pérdida de generación (contingencia N-1)", 500, 3000, 2000, 100,
            key="slider_forense", help="Simulación de pérdida de generación en MW"
        )

    inercias_sim = np.linspace(0.5, 6.0, 100)
    demanda_ref  = snap.get("demanda", 25_000) or 25_000
    rocof_sim    = [(dp * 50) / (2 * h * demanda_ref) for h in inercias_sim]

    # Límite normativo
    rocof_lim_norm = [1.0] * len(inercias_sim)
    rocof_lim_ufls = [0.5] * len(inercias_sim)
    ...
    fig_tray.add_hrect(
        y0=1.0, y1=3.5,
        fillcolor="rgba(231,76,60,0.08)", layer="below",
        annotation_text="Zona NC RfG crítica", annotation_position="top right",
        annotation_font=dict(color="#e74c3c", size=10),
    )
    fig_tray.add_hrect(
        y0=0.5, y1=1.0,
        fillcolor="rgba(243,156,18,0.06)", layer="below",
        annotation_text="Zona UFLS", annotation_position="top right",
        annotation_font=dict(color="#f39c12", size=10),
    )

    # Curva RoCoF simulado
    fig_tray.add_trace(go.Scatter(
        x=inercias_sim, y=rocof_sim,
        name=f"RoCoF simulado (ΔP={delta_p_slider} MW)",
        line=dict(color=COLOR_HOY, width=2.5),
        hovertemplate="H=%{x:.2f} s → RoCoF=%{y:.3f} Hz/s<extra></extra>",
    ))

    # Punto actual
    if snap.get("inercia"):
        rocof_actual_sim = rocof_ante_contingencia(
            snap["inercia"], delta_p_slider, demanda_ref
        )
        fig_tray.add_trace(go.Scatter(
            x=[snap["inercia"]], y=[rocof_actual_sim],
            name="Sistema actual",
            mode="markers",
            marker=dict(size=14, color=COLOR_HOY, symbol="circle",
                        line=dict(color="white", width=2)),
            hovertemplate=f"AHORA: H={snap['inercia']:.2f} s | RoCoF={rocof_actual_sim:.3f} Hz/s<extra></extra>",
        ))

    # Punto 28-A
    rocof_28a_sim = rocof_ante_contingencia(REF_28A["inercia"], delta_p_slider, demanda_ref)
    fig_tray.add_trace(go.Scatter(
        x=[REF_28A["inercia"]], y=[rocof_28a_sim],
        name="28-A (colapso)",
        mode="markers",
        marker=dict(size=14, color=COLOR_28A, symbol="x",
                    line=dict(color="white", width=2)),
        hovertemplate=f"28-A: H={REF_28A['inercia']:.2f} s | RoCoF={rocof_28a_sim:.3f} Hz/s<extra></extra>",
    ))

    # Umbrales
    fig_tray.add_hline(y=1.0, line=dict(color="#e74c3c", dash="dash"),
                       annotation_text="Umbral NC RfG (1.0 Hz/s)")
    fig_tray.add_hline(y=0.5, line=dict(color="#f39c12", dash="dot"),
                       annotation_text="Umbral UFLS (0.5 Hz/s)")

    fig_tray.update_layout(
        height=350,
        xaxis_title="Inercia del sistema H (s)",
        yaxis_title="RoCoF ante contingencia (Hz/s)",
    ...
    with col_info:
        if snap.get("inercia"):
            rocof_actual_s = rocof_ante_contingencia(snap["inercia"], delta_p_slider, demanda_ref)
            h_min = inercia_minima_segura(delta_p_slider, 1.0, demanda_ref)
            nivel_r = "🔴" if rocof_actual_s > 1.0 else "🟡" if rocof_actual_s > 0.5 else "🟢"
            st.metric("RoCoF simulado actual", f"{rocof_actual_s:.3f} Hz/s")
            st.metric("H mínima segura", f"{h_min:.3f} s",
                      delta=f"{(snap['inercia'] - h_min):+.3f} s margen")
            st.markdown(f"**Estado:** {nivel_r}")

    # ── Diagrama Sankey de flujos de balance ──
    st.markdown("#### 🌊 Diagrama de Flujos de Balance")

    demanda_s = snap.get("demanda", 25_000) or 25_000
    solar_s   = snap.get("solar",      0) or 0
    eolica_s  = snap.get("eolica",     0) or 0
    nuclear_s = snap.get("nuclear",    0) or 0
    ccgt_s    = snap.get("ccgt",       0) or 0
    hidro_s   = snap.get("hidraulica", 0) or 0
    cogene_s  = snap.get("cogeneracion",0) or 0
    fl_fr     = max(0, snap.get("flujo_francia", 0) or 0)
    fl_pt     = max(0, snap.get("flujo_portugal", 0) or 0)

    gen_total = solar_s + eolica_s + nuclear_s + ccgt_s + hidro_s + cogene_s + fl_fr + fl_pt

    # Nodos Sankey
    nodos = ["Solar", "Eólica", "Nuclear", "CCGT", "Hidráulica",
             "Cogeneración", "Importación FR", "Importación PT",
             "Generación Total", "Demanda"]
    idx   = {n: i for i, n in enumerate(nodos)}

    fuentes  = [idx["Solar"], idx["Eólica"], idx["Nuclear"], idx["CCGT"],
                idx["Hidráulica"], idx["Cogeneración"],
                idx["Importación FR"], idx["Importación PT"]]
    destinos = [idx["Generación Total"]] * 8
    valores  = [solar_s, eolica_s, nuclear_s, ccgt_s, hidro_s, cogene_s, fl_fr, fl_pt]
    fuentes  += [idx["Generación Total"]]
    destinos += [idx["Demanda"]]
    valores  += [gen_total]

    # Filtrar ceros
    mask = [v > 0 for v in valores]
    fuentes  = [f for f, m in zip(fuentes,  mask) if m]
    destinos = [d for d, m in zip(destinos, mask) if m]
    valores  = [v for v, m in zip(valores,  mask) if m]

    colores_nodo = [
        "#f1c40f","#2ecc71","#9b59b6","#e67e22","#3498db",
        "#e74c3c","#1abc9c","#16a085","#667eea","#e8eaf0"
    ]
    ...
    fig_sankey = go.Figure(go.Sankey(
        ...
        link=dict(
            ...
            hovertemplate="%{source.label} → %{target.label}: %{value:,.0f} MW<extra></extra>",
        ),
    ))
