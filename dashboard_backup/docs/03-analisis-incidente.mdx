...
import StickyTimeline from "@site/src/components/StickyTimeline";

<div className="timeline-sync-layout">
<div className="timeline-sync-content">
...
```mermaid
sequenceDiagram
    participant Red400 as Red 400 kV
    participant OLTC as Transformador con OLTC
    participant RedColectora as Red Colectora (220kV/132kV)
    participant IBR as Inversores (Plantas Solares/Eólicas)

    Red400->>OLTC: Transitorio rápido de sobretensión
    Note over OLTC: Retardo mecánico intencional<br/>para evitar oscilaciones efímeras
    OLTC--xRedColectora: Falla en ajustar la relación de transformación a tiempo
    Red400->>RedColectora: Propagación de sobretensión amplificada
    RedColectora->>IBR: Voltaje > 1.2 p.u.
    Note over IBR: Activación de protección de sobretensión<br/>(Función 59 ANSI)
    IBR->>RedColectora: Desconexión de plantas
    Note over Red400: El SCADA muestra tensión alta,<br/>pero bajo el límite crítico de 435 kV
```
